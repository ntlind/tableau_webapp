...
export default function Listbox({ options, selected, data, setData, alignTextRight = false, iconMap = null }: IProps) {
    return (
        <Menu as="div" className="relative inline-block text-left">
            <div>
                <Menu.Button className="inline-flex justify-center w-full px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm dark:bg-zinc-800 dark:hover:bg-zinc-700 dark:text-white ocus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-100 dark:focus:ring-offset-gray-500 focus:ring-theme">
                    {selected}
                                ...
                                {({ active }) => (
                                    <button
                                        className={classNames(
                                            active ? 'bg-zinc-100 text-gray-900 dark:bg-zinc-700' : 'text-gray-700 dark:bg-zinc-800',
